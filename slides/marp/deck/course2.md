...
問全班（講做法之後、揭曉之前）：「把整句攪成一杯平均，丟給上一堂那顆一個字都沒改的 MLP，猜猜看：它跑得出情緒嗎?」→ 讓覺得行和不行的都舉個手，再揭曉「居然會動」；有人說不行的話，記住他們，撞牆頁會回收這票人。
-->
...
問全班：唸完表之後真的問出來：「所以，不就 MLP 就好了嗎? 有人覺得哪裡怪怪的嗎?」→ 讓一兩個人講；沒人答出「順序」也沒關係，不要提示，直接開站讓他們自己撞。
-->
...
問全班（秀圖前先問）：「『故事』和『事故』，你分得出來嗎? 那剛剛那個詞袋模型呢?」→ 讓他們用剛玩過的站想一下：兩袋字一模一樣，平均完也一模一樣，模型連題目都拿到同一份。
-->
...
問全班：「這是資料不夠的問題嗎? 再餵它一百萬句，它分得出『故事』和『事故』嗎?」→ 分不出：順序在取平均那一步、進模型之前就被抹掉了，資料再多也救不回來，只能換架構。這題答對，這一節就到位了。
-->
