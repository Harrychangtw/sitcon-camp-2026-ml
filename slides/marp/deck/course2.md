...
- 點「他今天心情不好」，把「不」「好」對調成「好不」，看右邊通順度掉多少
...
帶站流程（台上示範）：
1. 指認介面：上排可拖曳的詞塊、打亂／還原鈕、左下詞袋指紋色帶、右下順序感知的通順度。
2. 示範一：點預設句「他今天心情不好」，按「打亂」兩三次 → 左邊色帶動也不動，右邊分數跟著跳。
3. 示範二：按「還原」，手動把「不」「好」對調成「…心情好不」→ 指紋還是不動，通順度明顯變差（原句是全部 120 種排列裡最通順的，實測 ppl 712 對 1162）。
4. 交棒放手 14 分鐘；巡場提示他們自己打一句再拖拖看。
（注意：不要臨場改用「今天天氣真不好」示範對調，實測它的「好不」版反而分數更高，會把牆演反。）
-->
