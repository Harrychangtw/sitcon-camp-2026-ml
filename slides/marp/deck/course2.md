...
<!--
講者備忘：verbatim spine：「今天的東西，第一次聽卡住很正常。有任何聽不懂的地方，隨時舉手打斷我。」開場先把規範立起來：舉手不用等我講完一段，被打斷是這堂課設計的一部分。講完停一拍再進 Loop 0。
自學備註：這頁是課堂約定，不含知識點。這堂課的節奏是「丟問題、動手、撞牆、給工具」，卡住的訊號越早浮出來越好，所以一開始就明說歡迎隨時打斷。
-->
...
<span class="chip">🛠 講師畫面／各組電腦已開好 · <a href="https://sitconcamp-gpu-v100x4.boreray-hippocampus.ts.net/tokenizer">/tokenizer</a></span>
...
<div class="anno" style="left:51.5%; top:90%; width:13%; height:6%;"><span>切分方式在這裡切換</span></div>
...
<!--
講者備忘：verbatim spine：「畫面長這樣，你要動的旋鈕在這裡。」開站前先給 5 秒鐘讓大家對照畫面，確認每個人都找到切分方式的按鈕再放手，不然找到位置時解說已經過了。
自學備註：站台畫面：上方輸入框打字，中間切換 字元／詞／BPE 三種切法，下方即時顯示彩色 token 切塊與每塊的編號。
-->
...
<!--
講者備忘：左邊是牆，右邊是解法，一頁對照完；右欄是 fragment，先只給 one-hot，問完「貓和狗的距離」那個問題、學生答完，再按出 embedding。one-hot 先講白話：整排都是 0，只有這個字自己那一格是 1，等於只回答「是哪個字」，沒有回答「像哪個字」。指著左圖問：這樣編碼，「貓」和「狗」的距離，跟「貓」和「桌子」的距離一樣嗎? 答案是一樣，這就是問題。右圖不寫公式，重點一句：語意 = 學出來的位置，距離近 = 意思近。
...
<span class="chip">🛠 講師畫面／各組電腦已開好 · <a href="https://sitconcamp-gpu-v100x4.boreray-hippocampus.ts.net/embedding">/embedding</a></span>
...
# 畫面長這樣 _Embedding 探索站_

<div class="shot">

![h:1250](../assets/stations/embedding.png)

<div class="anno" style="left:29.5%; top:85.5%; width:18%; height:7.5%;"><span>在這裡打字搜尋</span></div>

</div>

###### 打一個詞搜尋，點雲會亮出它和最近鄰；旁邊可切 2D／3D、調 Top K

<!--
講者備忘：verbatim spine：「畫面長這樣，先找到搜尋框。」開站前對照畫面 5 秒，確認大家找到搜尋框和 2D／3D 切換，再放手讓他們逛。
自學備註：站台畫面：搜尋框打一個詞，投影點雲會高亮它與最近鄰的位置，側欄列出最近鄰與相似度分數；可切 2D／3D、調 Top K。
-->
...
# 什麼都沒改就餵進去 _詞袋 bag-of-embeddings：把一句話拆成一袋字，不管順序，全部平均_
...
<span class="chip">🛠 講師畫面／各組電腦已開好 · <a href="https://sitconcamp-gpu-v100x4.boreray-hippocampus.ts.net/order-shuffle">/order-shuffle</a></span>
...
自學備註：打亂會把 token 的順序隨機重排。因為詞袋指紋是把每個 token 的 embedding 取平均，任何排列的平均都一樣，所以那條色帶不會變；同一句話右邊送給 Qwen 這種順序感知的模型，通順度與困惑度就會跟著順序變，兩相對照就看得出「有沒有把順序吃進去」的差別。
-->
...
![h:1250](../assets/stations/order-shuffle.png)

<div class="anno" style="left:54%; top:92.5%; width:6.5%; height:5.5%;"><span>打亂／還原在這裡</span></div>

</div>

###### 上面拖曳詞塊或按「打亂」，左下盯詞袋指紋，右下盯順序感知的通順度

<!--
講者備忘：verbatim spine：「畫面長這樣：按打亂之後，左右兩邊各盯一條。」開站前先指清楚三個位置：打亂鈕、左邊的詞袋指紋色帶、右邊的通順度分數，確認大家都找到了再放手。
自學備註：站台畫面：詞塊可拖曳重排，「打亂／還原」一鍵洗牌；左側是詞袋平均的指紋色帶（順序怎麼換都不動），右側是順序感知模型的通順度（跟著順序變）。
-->
...
<span class="chip">🛠 講師畫面／各組電腦已開好 · <a href="https://sitconcamp-gpu-v100x4.boreray-hippocampus.ts.net/next-token">/next-token</a></span>
...
# 畫面長這樣 _next-token 站_

<div class="shot">

![h:1250](../assets/stations/next-token.png)

<div class="anno" style="left:50%; top:81.5%; width:16.5%; height:6%;"><span>上下文視窗滑桿在這裡</span></div>

</div>

###### 上面打一句話，拖「上下文視窗」滑桿（就是剛剛的前文視窗），下面看候選字的機率條怎麼變

<!--
講者備忘：verbatim spine：「畫面長這樣，主旋鈕是這條上下文視窗滑桿，站上這樣叫，就是剛剛說的前文視窗。」開站前指清楚滑桿和機率條的位置，Temperature／Top-k 先不提，等他們玩起來再說。
自學備註：站台畫面：輸入框打一句話，GPU 上的 Qwen3 即時列出候選 token 的機率條；前文視窗滑桿決定模型能看到最後幾個 token，是本站的主旋鈕。
-->
...
<span class="chip">🛠 講師畫面／各組電腦已開好 · <a href="https://sitconcamp-gpu-v100x4.boreray-hippocampus.ts.net/rnn-viz">/rnn-viz</a></span>
...
![h:1250](../assets/stations/rnn-viz.png)

<div class="anno" style="left:36%; top:59.3%; width:29.5%; height:4%;"><span>盯住這條「影響」列</span></div>

</div>

###### 拖上面的滑桿逐字前進；記憶一列列往下填，最底那條「影響」列越早的字越淡

<!--
講者備忘：verbatim spine：「畫面長這樣，等一下你要盯的是最底下這條影響列。」開站前把滑桿和影響列指清楚，尤其影響列，牆就藏在那裡，不先指出來學生會只顧著拖滑桿。
自學備註：站台畫面：拖曳滑桿逐 token 前進，hidden state 在格子圖上一列列填出；最底下的「影響」列顯示每個先前 token 對當前記憶的參考強度，越早的 token 顏色越淡。
-->
...
滑過**注意力矩陣**（attention matrix）的格子，看某個字把多少注意力分給另一個字；再轉 Layer／Head 換層換頭。

</div>
<div class="st">
<h4>試試看</h4>

- 滑過矩陣格子，找代名詞那一列，看它把注意力分給誰
...
每個字直接對整句算注意力（causal：只看得到左邊），相關的字一格就連上，沒有逐站接力。
...
<span class="chip">🛠 講師畫面／各組電腦已開好 · <a href="https://sitconcamp-gpu-v100x4.boreray-hippocampus.ts.net/transformer">/transformer</a></span>
...
# 畫面長這樣 _Transformer 站_

<div class="shot">

![h:1250](../assets/stations/transformer.png)

<div class="anno" style="left:48%; top:35%; width:11%; height:19.5%;"><span>滑這個注意力矩陣的格子</span></div>

</div>

###### 生產線往右捲：輸入 → 切詞 → embedding → 注意力矩陣 → 下一個字的機率；Layer／Head 轉盤在矩陣旁邊

<!--
講者備忘：verbatim spine：「畫面長這樣，往右捲，你要玩的是中間這塊注意力矩陣。」這站畫面最複雜，開站前一定要先給畫面：整條生產線先掃一次，再把注意力矩陣和 Layer／Head 轉盤指清楚，確認每個人都找到矩陣再放手，不然學生剛找到位置就錯過解說。
自學備註：站台畫面是一條可橫向捲動的 forward pass 生產線：輸入 → tokenizer → embedding → attention matrix ＋ MLP → next-token 機率。滑過矩陣的格子會高亮 query／key 兩個 token 並顯示權重；矩陣旁有 Layer／Head／Temperature 三個轉盤。
-->
...
_圖示：關掉捷徑，深層 loss 亂跳；補上捷徑，訓練就穩。_
...
<!--
講者備忘：四個膠囊逐步出現（fragment），一拍一塊。收束用，把整個 Loop 3 拼回一張圖：一個機制（attention）加三塊補丁
