...
- **橋接**：把一句話的 token embedding 平均起來（bag-of-embeddings）丟進上一堂的 MLP，做國會資料的情感分析 → **它居然會動、準度還不錯** → 製造假安全感：「那不就 MLP 就好？」
- **撞牆 demo（環境引導關鍵）**：一個「打散字序」開關。
    - 學員動的旋鈕：shuffle on/off、切換 MLP(bag) / RNN 重跑。
    - 觀察：**MLP(bag) shuffle 前後輸出逐字相同** → 它把順序整個丟掉了，對它來說句子只是一袋無序的字。
    - 對照：挑 order 有訊號的例子（「不好」vs「好不」）→ MLP(bag) 卡在天花板。
- 收束：MLP 的設計裡根本沒有「順序」這個假設 → 我們需要一個**假設順序有意義**的架構 → RNN。
...
- **順序撞牆站（取代原 CNN pixel-shuffle）**：字序 shuffle 開關、bag-of-embeddings+MLP / RNN 切換、兩者準度即時對比。
