...
| 2 | Model architecture evolution: MLP → RNN → Transformer | **in progress** | `tokenizer`, `embedding`, `order-shuffle`, `next-token`, `rnn-viz`, `transformer` (all placeholders) + dev: `/_reference`, `/viz-sandbox` |
| 3 | TBD | not built | — |
