...
| `GET /leaderboard` | — | Session required (any role — it powers the projector view). `{individuals, teams, questTotals, generatedAt}`, students only in the rankings, sorted by points desc with earlier-last-score tiebreak. 小隊 labels come from `GROUPS_CSV`; unmapped students rank under `未分組`. |
